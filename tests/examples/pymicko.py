...
class SymbolTableEntry:
    ...
    def attribute_str(self):
        ...
        return f"{self.attribute_name}={self.attribute}" if self.attribute != None else "None"
    ...
    def display(self):
        ...
        for i,sym in enumerate(self.table):
            ...
            for p in sym.param_types:
                if parameters == "":
                    parameters = f"{SharedData.TYPES[p]}"
                else:
                    parameters += f", {SharedData.TYPES[p]}"
...
class CodeGenerator:
    ...
    def arithmetic(self, operation, operand1, operand2, operand3 = None):
        ...
        self.newline_text(f"{mnemonic}\t{self.symbol(operand1)},{self.symbol(operand2)},{self.symbol(output)}", True)
        return output
    ...
    def move(self,operand1, operand2):
        ...
        self.newline_text(f"MOV \t{self.symbol(operand1)},{self.symbol(operand2)}", True)
        if isinstance(operand2, int):
    ...
    def compare(self, operand1, operand2):
        ...
        self.free_if_register(operand1)
        self.free_if_register(operand2)
        self.newline_text(f"CMP{self.OPSIGNS[typ]}\t{self.symbol(operand1)},{self.symbol(operand2)}", True)
    ...
    def function_body(self):
        ...
        if self.shared.function_vars > 0:
            const = self.symtab.insert_constant(f"0{self.shared.function_vars * 4}", SharedData.TYPES.UNSIGNED)
    ...
    def function_call(self, function, arguments):
        ...
        if args > 0:
            args_space = self.symtab.insert_constant(f"{args * 4}", SharedData.TYPES.UNSIGNED)
...
class MicroC:
    ...
    def relexp_action(self, text, loc, arg):
        ...
        if not self.symtab.same_types(arg[0], arg[2]):
            raise SemanticException(f"Invalid operands for operator '{arg[1]}'")
    ...
    def andexp_action(self, text, loc, arg):
        ...
        if DEBUG > 0:
            ...
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        label = self.codegen.label(f"false{self.false_label_number}", True, False)
    ...
    def logexp_action(self, text, loc, arg):
        ...
        if DEBUG > 0:
            ...
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        label = self.codegen.label(f"true{self.label_number}", True, False)
        self.codegen.jump(self.relexp_code, False, label)
        self.codegen.newline_label(f"false{self.false_label_number}", True, True)
        self.false_label_number += 1
    ...
    def if_begin_action(self, text, loc, arg):
        ...
        self.codegen.newline_label(f"if{self.label_number}", True, True)
    ...
    def if_body_action(self, text, loc, arg):
        ...
        self.codegen.newline_label(f"true{self.label_number}", True, True)
    ...
    def if_else_action(self, text, loc, arg):
        ...
        label = self.codegen.label(f"exit{self.label_number}", True, False)
        self.codegen.unconditional_jump(label)
        #generate final 'false' label (executes if condition isn't satisfied)
        self.codegen.newline_label(f"false{self.label_stack.pop()}", True, True)
        self.label_stack.append(self.label_number)
    ...
    def if_end_action(self, text, loc, arg):
        ...
        if DEBUG > 0:
            ...
            if DEBUG == 2: self.symtab.display()
            if DEBUG > 2: return
        self.codegen.newline_label(f"exit{self.label_stack.pop()}", True, True)
    ...
    def while_begin_action(self, text, loc, arg):
        ...
        self.false_label_number += 1
        self.label_number = self.false_label_number
        self.codegen.newline_label(f"while{self.label_number}", True, True)
    ...
    def while_end_action(self, text, loc, arg):
        ...
        self.label_number = self.label_stack.pop()
        label = self.codegen.label(f"while{self.label_number}", True, False)
        self.codegen.unconditional_jump(label)
        #generate final 'false' label and exit label
        self.codegen.newline_label(f"false{self.label_stack.pop()}", True, True)
        self.codegen.newline_label(f"exit{self.label_number}", True, True)
...
if 0:
    ...
    If input file is omitted, stdin is used""".format(argv[0])
        ...
        exit(1)
    try:
        parse = stdin if input_file == stdin else open(input_file)
    except Exception:
