...
print(f"{module} = CDLL('{module}.dll')")
print()
print("# user defined types")
for tdname,tdtyp in typedefs:
    print(f"{tdname} = {typemap[tdtyp]}")
for fntd in fn_typedefs:
...
print("# functions")
for fn in functions:
    prefix = f"{module}.{fn.fn_name}"

    print(f"{prefix}.restype = {typeAsCtypes(fn.fn_type)}")
    if fn.varargs:
